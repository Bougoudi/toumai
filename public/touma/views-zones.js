...
export async function serviceZones() {
  ...
  if (!mine.items.length) {
    return `<h1 style="font-size:var(--text-xl)">${esc(t('zone.title'))}</h1>${tabs('/touma/vendeur/zones')}
      <div class="card"><p class="muted">${esc(t('zone.needStore'))}</p></div>`;
  }
  ...
  return `<h1 style="font-size:var(--text-xl)">${esc(t('zone.title'))}</h1>
    ...
    ${
      zones.unrestricted
        ? `<div class="alert">
            <div>
              <strong>${esc(t('zone.unrestrictedTitle'))}</strong>
              <div class="small">${esc(t('zone.unrestrictedBody'))}</div>
            </div>
          </div>`
        : ''
    }
    ...
    <form class="card" id="service-zones-form" data-store="${esc(store.id)}">
      <h2 style="font-size:var(--text-base);margin-bottom:var(--space-3)">${esc(t('zone.storeChad', { store: store.name }))}</h2>
      ...
      <label class="row" style="gap:var(--space-2);align-items:center;margin-bottom:var(--space-3)">
        ...
        <span>${esc(t('zone.allChad'))}</span>
      </label>

      <div class="field" style="max-width:16rem">
        <label for="sz-handling">${esc(t('zone.handlingLabel'))}</label>
        <input id="sz-handling" type="number" min="0" max="90" step="1"
               value="${nationale?.handlingDays ?? ''}" placeholder="${esc(t('zone.handlingPlaceholder'))}" inputmode="numeric" />
        <p class="xs muted">${esc(t('zone.handlingHint'))}</p>
      </div>

      <h3 style="font-size:var(--text-base);margin:var(--space-4) 0 var(--space-2)">${esc(t('zone.provinces'))}</h3>
      <p class="xs muted" style="margin-bottom:var(--space-3)">${esc(t('zone.provincesHint'))}</p>
          ...
          .map((p) => {
            ...
            return `<div class="row" style="gap:var(--space-3);align-items:center;flex-wrap:wrap">
              ...
              <input class="input sz-note" data-note="${esc(p.id)}" style="flex:1;min-width:200px"
                     placeholder="${esc(t('zone.notePlaceholder'))}"
      ...
      <div class="row" style="gap:var(--space-2);margin-top:var(--space-4)">
        <button class="btn btn-primary" type="submit">${esc(t('action.save'))}</button>
        <button class="btn btn-ghost" type="button" data-clear-zones="${esc(store.id)}">${esc(t('zone.clearAll'))}</button>
      </div>
    </form>

    <p class="small muted" style="margin-top:var(--space-4)">${esc(t('zone.footnote'))}</p>`;
}
